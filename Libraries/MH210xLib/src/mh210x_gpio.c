...
#define MH_REMAP_FSMC_MASK			((uint32_t)0xC0000000)
...
{
	/* Check the parameters */
	assert_param(IS_MH_GPIO_REMAP(GPIO_Remap));
	assert_param(IS_FUNCTIONAL_STATE(NewState));  
	
	if(GPIO_Remap == GPIO_Remap_FSMC1 | GPIO_Remap == GPIO_Remap_FSMC2)
	{
		AFIO->MAPR2 = AFIO->MAPR2 & ~MH_REMAP_FSMC_MASK;
...
/**
  ...
  * @note   This function applies only to mh Connectivity line devices.  
